...
% Example_ann_line_plot.m
% to plot Figure 4 in the paper:
...
% generat a discretization of intervals: (-1,1)\[-0.25,0.25]
s=-1:0.001:1;
s(abs(s)>=1)     = NaN;
s(abs(s)<=0.25)   = NaN;
sum(sum(abs(s)>=0))
...
% 
% save the data
save('san5_pt.mat',  's', '-ascii', '-double')
save('Rsan5_pt.mat', 'R', '-ascii', '-double')
%